...
submitRegistration(): Observable<any> {
  ...
  formData.append('personalInfo', JSON.stringify(this.hostregistrationData.personalInfo))
  formData.append('addressDetails', JSON.stringify(this.hostregistrationData.addressDetails))
  formData.append('propertyDetails', JSON.stringify(this.hostregistrationData.propertyDetails))
  formData.append('petInfo', JSON.stringify(this.hostregistrationData.petInfo))
  formData.append('pricing', JSON.stringify(this.hostregistrationData.pricing))
  formData.append('idVerification', JSON.stringify(this.hostregistrationData.idVerification))

  if(this.hostregistrationData.propertyDetails?.propertyPhotos){
    this.hostregistrationData.propertyDetails.propertyPhotos.forEach((file, index) => {
      formData.append(`propertyPhoto_${index}`, file)
    })
  }

  if(this.hostregistrationData.idVerification?.frontIdFile){
    formData.append('frontId', this.hostregistrationData.idVerification.frontIdFile)
  }

  
  if(this.hostregistrationData.idVerification?.backIdFile){
    formData.append('backId', this.hostregistrationData.idVerification.backIdFile)
  }
